...
def save_npy_to_pth(model_name: str, npy_path: str, pth_path: str):
    ...
    try:
        ...
        if "detector" in model_name.lower():
            ...
            tokenizer = CharTokenizer.load("data/seq2seq_char_tokenizer.json")
    ...
    def aggregate_evaluate(
        ...
        if primary in metrics_agg and metrics_agg[primary] > self.best_metric:
            ...
            if os.path.exists(best_src):
                ...
                pth_dest = "models/detector_best.pth" if is_detector else "models/seq2seq_best.pth"
                save_npy_to_pth(self.model_name, best_src, pth_dest)
                if is_detector:
                    shutil.copy(pth_dest, "models/detector_best.pth.pth")
...
class DecoderWrapper(torch.nn.Module):
    ...
    def forward(self, token, h, c, enc_out):
        pred, new_h, new_c, _ = self.decoder.forward_step(token, h, c, enc_out)
...
def load_model(model_type: str):
    ...
    if model_type == "detector":
        ...
        if not os.path.exists(path) and os.path.exists("models/detector_best.pth.pth"):
            path = "models/detector_best.pth.pth"
...
def export_corrector_encoder_onnx():
    ...
    try:
        ...
        model.eval()
        buffer = io.BytesIO()
        dummy_x = torch.zeros((1, 30), dtype=torch.long)
        torch.onnx.export(
            model.encoder,
            (dummy_x,),
            buffer,
...
def export_corrector_decoder_onnx():
    ...
    try:
        ...
        dummy_enc_out = torch.zeros((1, 30, 128), dtype=torch.float)
        torch.onnx.export(
            wrapper,
...
async def import_onnx(model_type: str, file: UploadFile = File(...)):
    ...
    try:
        ...
        dest_path = "models/detector_best.pth" if model_type == "detector" else "models/seq2seq_best.pth"
