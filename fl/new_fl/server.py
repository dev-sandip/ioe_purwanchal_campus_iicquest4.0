...
S2S_TOK           = ROOT / "data" / "nepali_char_tokenizer.json"
TOKENIZER_VOCAB   = ROOT / "data" / "nepali_vocab.json"
...
# ── model builders ─────────────────────────────────────────────────────────────
def _build_corrector(tok: NepaliCharTokenizer, dropout=0.1) -> NepaliSeq2SeqCorrector:
    """
    Build Nepali Seq2Seq Corrector with hyperparams from notebook training.
    
    Args:
        tok: NepaliCharTokenizer instance
        dropout: Dropout rate (default 0.1)
    
    Returns:
        NepaliSeq2SeqCorrector model
    """
    return NepaliSeq2SeqCorrector(
...
def export_onnx():
    ...
    Model Input:
      ...
      tgt: int64[batch, max_len=100]    (target char IDs for teacher forcing)
    
    Model Output:
      logits: float32[batch, max_len, vocab_size]  (correction logits)
    """
    try:
        tok   = NepaliCharTokenizer.load(str(S2S_TOK))
...
class NepaliCorrectionFedAvg(fl.server.strategy.FedAvg):
    """
    FedAvg for Nepali Seq2Seq Correction model.
    ...
    def _save_best(self, round_num: int):
        ...
        try:
            ...
            # 3. rebuild Nepali seq2seq model
            tok   = NepaliCharTokenizer.load(str(S2S_TOK))
            model = _build_corrector(tok)
