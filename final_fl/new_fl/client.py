...
Clients train NepaliCorrectionModel locally on their data
...
from new_fl.data_utils import load_pairs, build_correction_dataset
...
class NepaliCorrectionFlowerClient(fl.client.NumPyClient):
    """
    Flower NumPyClient for Seq2Seq Grammar Correction.
    
    Each client maintains a local NepaliCorrectionModel,
    ...
    def __init__(
        ...
        Args:
            model: NepaliCorrectionModel instance
    ...
    def evaluate(
        ...
        with torch.no_grad():
            for src, tgt in self.val_loader:
                ...
                logits = self.model(src, tgt, teacher_forcing_ratio=0.0)
                loss = self.criterion(
                    logits[:, 1:].reshape(-1, logits.size(-1)),
                    tgt[:, 1:].reshape(-1)
                )
...
def start_client(
    ...
    client_id: int = 0,
    csv_path: str = "data/right_wrong.csv",
    tokenizer_path: str = "models/nepali_correction_tokenizer.json",
    batch_size: int = 32,
    train_split: float = 0.8,
):
    """
    Start a Flower FL client for Grammar Correction.
    
    Args:
        server_address: Server host:port
        client_id: Client identifier
        csv_path: Path to correction pairs CSV
        tokenizer_path: Path to saved CharTokenizer
...
if __name__ == "__main__":
    ...
    parser.add_argument("--csv", default="data/right_wrong.csv")
    parser.add_argument("--tokenizer", default="models/nepali_correction_tokenizer.json")
